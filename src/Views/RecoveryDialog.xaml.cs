...
public sealed partial class RecoveryDialog : ContentDialog
{
    ...
    public RecoveryDialog(Window owner, IVaultService vault, long entryId, string label) : this()
    {
    ...
    private void Reload()
    {
        ...
        KeyList.ItemsSource = _vault.GetRecovery(_entryId);
        NewKeyBox.Text = "";
    ...
    {
        var rc = _vault!.SetRecovery(_entryId, keys);
        if (rc != ErrorCodes.Ok)
        {
    ...
    private void AddKey()
    {
        ...
        var keys = _vault.GetRecovery(_entryId);
