...
public sealed partial class EntryDialog : ContentDialog
{
    ...
    public EntryDialog(Window owner, IVaultService vault, EntryDetail? existing = null,
        long? preselectCatId = null) : this()
    {
        _owner = owner;
        XamlRoot = owner.Content.XamlRoot;
        _vault = vault;
        Init(vault, existing, preselectCatId);
    }

    // preselectCatId：新增时预选当前浏览的分类（null 时不预选，保存时归入未分类）
    internal void Init(IVaultService vault, EntryDetail? existing = null, long? preselectCatId = null)
    {
        _vault = vault;
        _categories = vault.ListCategories() ?? new();
        ...
        if (selected.Count > 1)
        {
            ...
            _multiCats = selected.Where(id => id != VaultStore.UncatId).ToList();
    ...
    private List<long> CurrentSelection() => _multiMode
        ? _multiCats
        : (CategoryCombo.SelectedItem is Category c && c.Id != VaultStore.UncatId
            ? new List<long> { c.Id }
            : new List<long>());
    ...
    private void MultiCatBtn_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            if (c.Id == VaultStore.UncatId) continue; // 本面板不展示「未分类」
    ...
    // AddCategory 在重名/失败时返回 ErrorCodes 枚举值（正数），与新分类 Id 同为正数区间，
    // 因此这里先自行查重，避免把错误码误判为新 Id。
    private long? TryCreateCategory(string text, out string error)
    {
        ...
        long rc = _vault!.AddCategory(name);
        if (rc <= 0)
        {
            error = rc == -(long)ErrorCodes.Dup ? "已存在同名分类。" : "新建分类失败。";
            return null;
        }
        _vault.Save();
        _categories = _vault.ListCategories() ?? _categories;
        CategoryCombo.ItemsSource = _categories;
        return rc;
    }
    ...
    private void OnPrimaryButtonClick(ContentDialog sender, ContentDialogButtonClickEventArgs args)
    {
        ...
        var cats = _multiMode
            ? _multiCats.ToList()
            : (CategoryCombo.SelectedItem is Category c ? new List<long> { c.Id } : new List<long>());
        if (cats.Count == 0) cats.Add(VaultStore.UncatId); // 未选择时归入内置「未分类」
        CategoryIds = cats;
