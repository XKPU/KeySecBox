...
namespace KeySecBox
{
    public sealed partial class VaultPage : Page
    {
            ...
            string account, string password, string note, long? categoryId)
        {
            ...
            var accBox = new TextBox { Text = account, PlaceholderText = "账户名" };
            var pwdBox = new PasswordBox { Password = password, PlaceholderText = "密码" };
            var noteBox = new TextBox { Text = note, PlaceholderText = "备注（可选）", AcceptsReturn = true, Height = 60 };
            var combo = new ComboBox
            {
                Width = 180, DisplayMemberPath = "Name", SelectedValuePath = "Id", ItemsSource = _categories
            };
            ...
            var err = new TextBlock
            {
                ...
                FontSize = 12,
                Foreground = new Microsoft.UI.Xaml.Media.SolidColorBrush(Windows.UI.Color.FromArgb(255, 0xC4, 0x2B, 0x1C))
            };
            ...
            try { save.Style = (Style)Application.Current.Resources["AccentButtonStyle"]; } catch { }
        ...
        private static StackPanel Labeled(string label, UIElement input)
        {
            ...
            sp.Children.Add(new TextBlock
            {
                Text = label,
                FontSize = 12,
                Foreground = new Microsoft.UI.Xaml.Media.SolidColorBrush(Windows.UI.Color.FromArgb(255, 0x88, 0x88, 0x88))
            });
        ...
        private void SearchToggleBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            Storyboard.SetTarget(width, SearchBox);
            ...
            Storyboard.SetTarget(fade, SearchBox);
            Storyboard.SetTargetProperty(fade, "Opacity");
            sb.Children.Add(fade);
        ...
        private void EntryRecoveryBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (keys is { Count: > 0 })
            {
                ...
                for (int i = 0; i < keys.Count; i++) fields.Add(($"密钥 {i + 1}", keys[i]));
