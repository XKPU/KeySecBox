...
- Visual Studio（勾选「使用 C++ 的桌面开发」工作负载，自带 CMake 与 MSVC），或独立安装 CMake >= 3.20 + MSVC Build Tools
- .NET 8 SDK
